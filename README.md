...
All six agents use `claude-opus-4-7`. The Harmonizer's `max_tokens` is set to 8192 for long-form synthesis; the Country Agents use structured JSON output; the Conflict Extractor and Confidence Grader return strictly JSON schemas and fail gracefully to empty output without blocking the primary result.
